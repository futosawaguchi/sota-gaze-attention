...
| `SOTA_WAIST_ENABLED` | 360°化（頭＋腰）を使う（後方も体ごと向く） | 0（無効） |
| `SOTA_PULSE_PER_DEG_WAIST` | 腰の度→パルス係数 | 10（1200/120） |
| `SOTA_HEAD_YAW_MAX_DEG` | 頭が担う最大ヨー（度）。超過分は腰へ | 80 |
| `SOTA_WAIST_YAW_MAX_DEG` | 腰が担う最大ヨー（度） | 120 |
| `SOTA_WAIST_SIGN` | 腰の回転が逆なら `-1` | 1 |
...
| **後方を向けない/端で止まる** | 頭のみ（腰未使用） | `SOTA_WAIST_ENABLED=1`（§6） |
| **腰が逆/回りすぎ** | 腰の符号/gain | `SOTA_WAIST_SIGN=-1` / `SOTA_PULSE_PER_DEG_WAIST` |
| **小さい視線でも体が回る** | 頭の担当が狭い | `SOTA_HEAD_YAW_MAX_DEG` を上げる |
...
`gaze_yaw` は**絶対方位**なので「Sota を gaze_yaw に向ける」一規則で前後とも整合する（場合分け不要）。
ただし頭ヨー（`Head_Y` ±1400）だけでは 360° を覆えない。**腰 `Waist_Y`（±1200）で体ごと回し**、
頭の可動域（`SOTA_HEAD_YAW_MAX_DEG`）を超えた分を腰が補う（頭優先・腰は超過分）。Java 側は改修不要。

校正手順:

1. **腰の符号と可動域**を gaze 無しで確認:
   ```bash
   python scripts/sota_poke.py --waist-y 400     # 体が右に回る? 左? → 逆なら SOTA_WAIST_SIGN=-1
   python scripts/sota_poke.py --waist-y 1200    # 端まで（±1200 が実角度で何度か実測）
   ```
   実測した「腰 ±1200 ↔ ±◯度」から `SOTA_PULSE_PER_DEG_WAIST = 1200/◯` を設定。
2. **有効化**: `.env` に `SOTA_WAIST_ENABLED=1`。
3. **受け渡し点**: `SOTA_HEAD_YAW_MAX_DEG`（既定 80）まで頭、超過は腰。小さく見るのは頭・大きく向くのは体、になるよう調整。
4. **到達範囲**: `SOTA_HEAD_YAW_MAX_DEG + SOTA_WAIST_YAW_MAX_DEG ≥ 180` なら理論上は真後ろまで届く（実機の腰可動角で決まる）。
5. `python app.py --local` で、人が**後方**を見た時に Sota が体ごと向き直るか確認。

> 動きの考え方: 頭が `head_yaw_max` で飽和したら腰が回る。境界付近のガタつきは `SOTA_EMA_ALPHA` / `SOTA_DEADBAND_DEG` で抑える。

## 7. 参考: そのまま動く道具

```bash
python scripts/sota_poke.py --sweep         # 符号・可動域の素振り（gaze 不要）
python scripts/sota_poke.py --head-y 500    # 単発（頭）
python scripts/sota_poke.py --waist-y 400   # 単発（腰）
